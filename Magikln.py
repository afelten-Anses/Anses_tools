...
def get_parser():
    ...
    parser.add_argument('-p', action="store", dest='project',
						type=str, required=True, help='project name (REQUIRED)')
    
    parser.add_argument('-g', action="store", dest='pathogene', 
                        choices=['Salmonella','Listeria','Bacillus','Clostridium','Staphylococcus'],
						type=str, required=True, help='pathogene (REQUIRED)')
...
def main():
    ...
    if genomes.find({"Project":Arguments.project}).count() == 0:
        print("ERROR : project " + Arguments.project + " doesn't exist in GAMeRdb !")
        sys.exit(1)

    listDoc = genomes.find({"Project":Arguments.project, "Phylogeny.Genus":Arguments.pathogene})
