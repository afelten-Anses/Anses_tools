	...
	for element in os.listdir(Arguments.directory):
        ...
        for file in files :
            ...
            if ".msh" in file :
                ...
                id_genome = file.split("_")[0]

        if not flag :

            os.system("cp " + assembly_path + " .")
            cmd = "mash sketch -p " + str(Arguments.NbThreads) + " -k " + Arguments.Ksize + " -s " + \
            Arguments.Ssize + " -r -o " + id_genome + " " + assembly
            os.system(cmd)    
            os.system("mv " + id_genome + ".msh " + Arguments.directory + '/' + element + '/.')
            os.system("rm assembly")

            #genomes.update({'SampleID':id_genome},{"$set":{'Sketch':id_genome + ".msh"}},upsert=True)
